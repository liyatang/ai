...
for scenario in ["normal","recovered","failure","stale","missing","long","dns","dns_tls","tls"] {
    let card = CardView()
    let failing = scenario == "failure"
    let long = scenario == "long"
    let stale = scenario == "stale"
    if scenario != "missing" {
        let dns = scenario == "dns" || scenario == "dns_tls"
        let tls = scenario == "tls"
        let diagnosis = Diagnosis(status:failing ? "sustained" : "clear",severity:failing ? "danger" : dns || tls ? "warning" : "good",
            title:dns ? (scenario == "dns_tls" ? "连接异常，检测到 DNS 解析失败" : "DNS 解析异常") : tls ? "TLS 握手异常" : failing ? "连接持续异常" : scenario == "recovered" ? "重试后已继续输出" : "近期未见异常",
            advice:dns ? "检查 Clash DNS 设置及上游可达性" : tls ? "检查 TLS 连接与代理链路" : failing || long ? "可尝试：新加坡 03 · 高速专线与长期候选节点名称" : "无需调整，继续使用",
            ...
            short_title:scenario == "dns_tls" ? "连接异常 · DNS 解析失败" : nil)
        ...
        for i in 0..<8 { card.recordLatencySample(latencyMs:failing || dns || tls ? nil : 350+Double(i%3)*90,ok:!(failing || dns || tls),at:Date(timeIntervalSince1970:now-Double(7-i)*23)) }
    }
    for i in 0..<90 {
