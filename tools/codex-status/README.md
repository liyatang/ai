...
官方连接检测覆盖 `chatgpt.com`、`ws.chatgpt.com`、`api.openai.com`。Clash DNS 每个域名单独判断：连续两轮明确解析错误才提醒，连续两轮成功后撤下；接口不可用、过期、空地址结果均保留未知。Fake-IP 是正常模式，不能据此判断故障。

短请求记录 DNS、连接、TLS、超时或 HTTP 阶段；HTTP 401／403 代表收到响应，不是 DNS 失败。仅 `chatgpt.com` 有主动 HTTPS 短请求证据，其余域名的 DNS 失败不冒充已观察到的请求失败。TLS 错误不证明 DNS 故障，短请求正常也不证明模型速度正常。

DNS 活跃时每 60 秒、空闲时每 120 秒独立采集；单查询最多 3 秒、整轮最多 5 秒。节点、配置文件元数据变化和休眠恢复会隔离旧观察；DNS 配置仅检查文件元数据，不读取认证或订阅内容。控制器以其他方式热更新且不反映在可观察身份中时，不能保证立即识别该变化。

提醒显示在现有卡片结论及悬停详情；独立的 Codex 持续重试仍可显示红色。DNS 异常期间优先提示检查 DNS，不继续展示节点推荐。不修改 Clash、不清缓存、不固定真实 IP、不调用模型测速、不新增系统通知。
