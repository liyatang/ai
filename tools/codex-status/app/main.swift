...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    var quotaGate = RefreshGate(), diagnosticGate = RefreshGate(), benchmarkGate = RefreshGate()
    ...
    func input() -> DiagnosticInput {
        DiagnosticInput(session_id:sessionID,epoch:epoch,samples:samples,benchmark:benchmarkData)
    }
    func resetObservation() {
        sessionID = UUID().uuidString; epoch = nil; generation.invalidate()
        samples = []; benchmarkData = nil; card.clearLatencySamples(); card.networkRateSamples = []
    ...
    func acceptDiagnostics(_ result: DiagnosticsData?, token: Int) -> Bool {
        ...
        if epoch != result.epoch {
            generation.invalidate(); samples = []; benchmarkData = nil
    ...
    func maybeProbe() {
        ...
        if probe.start({ [weak self] latency,ok in
            guard let self, self.generation.accepts(token), self.epoch == epoch else { return }
            let sample = ProbeSample(at:Date().timeIntervalSince1970,latency_ms:ok ? latency : nil,ok:ok)
            self.samples.append(sample); self.samples = Array(self.samples.suffix(16))
            self.card.recordLatencySample(latencyMs:sample.latency_ms,ok:ok)
